    ...
    if (user) {
      return (
        <>
          ...
          &nbsp;
          <a href={"/EditMedico/" + rowData.id} className="btn btn-primary">
            Editar
          </a>
          &nbsp;
          <a
            value={rowData.id}
            href={"medico/" + rowData.id}
  ...
  async function eliminar(e) {
    ...
    if (confirmacion) {
      ...
      const response = await fetch(
        "http://127.0.0.1:8000/api/medicos/delete/" + e.target.value + "/",
        { method: "DELETE" }
      )
